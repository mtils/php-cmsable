...
namespace Cmsable\Foundation\Http;
...
use Ems\Contracts\Core\InputCaster;
use Ems\Contracts\Core\InputCorrector;
                                                ...
                                                ReceivesDistributorWhenResolved
{
    ...
    /**
     * @var \Ems\Contracts\Core\InputCorrector
     **/
    protected $corrector;

    /**
     * @var \Ems\Contracts\Core\InputCaster
     **/
    protected $caster;
    ...
    public function cleaned($key=null, $default=null)
    {
        ...
        if ($this->casted === null) {
            ...
            $corrected = $this->performCorrection($all);

            $this->performValidation($corrected, $this->model);

            $this->casted = $this->performCasting($corrected);
    ...
    protected function &performCorrection(array $parameters)
    {
        $corrected = $this->corrector()->correct($parameters, [], $this->resourceName());
        $this->publish('input.corrected', [&$corrected]);
        return $corrected;
    }

    protected function corrector()
    {
        if (!$this->corrector) {
            $this->corrector = $this->container->make(InputCorrector::class);
        };
    ...
    protected function &performCasting(array $parameters)
    {
        ...
        $casted = $this->caster()->cast($parameters, [], $this->resourceName());
    ...
    {
        if (!$this->caster) {
            $this->caster = $this->container->make(InputCaster::class);
        }
        return $this->caster;
    }
